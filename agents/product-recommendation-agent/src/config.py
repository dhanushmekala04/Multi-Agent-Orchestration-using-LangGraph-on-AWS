"""
Configuration management for the product recommendation agent service.
"""
...
class ProductRecommendationConfig:
    """Configuration class for Product Recommendation Agent."""
    ...
    def __init__(self):
        ...
        self.dynamodb_endpoint_url = os.getenv("DYNAMODB_ENDPOINT_URL")  # None for AWS service
    ...
    def _get_default_claude_37_inference_profile(self) -> str:
        ...
        if region in claude_37_profiles:
            ...
            print(
                f"✅ Product Recommendation Agent: Using Claude 3.7 Sonnet cross-region inference profile for {region}: {profile_id}"
            )
            print(
            ...
            print(
                f"⚠️  Product Recommendation Agent: Region {region} not configured for Claude 3.7 Sonnet cross-region profiles"
            )
...
def setup_logging(config: ProductRecommendationConfig):
...
config = ProductRecommendationConfig()
